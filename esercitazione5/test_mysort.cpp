...
int test_sort(int dimensione, T val_min, T val_max){
	...
	my_sort(v, 0, v.size()-1);
	int c = issorted(v);
	return c;
}

int main(){
	cout << endl << "Si testa l'algoritmo My_sort su 100 vettori di dimensioni randomiche da 1 a 8000 riempiti di valori randomici da -10000 a +10000 interi, double e float." <<endl;
	double tempo=0;
	randfiller rf;
